...
void Game::load_assets(Renderer &renderer)
{
    ...
    auto *tree = new Object{ renderer, 100, 100, resource_blank, resource_blank_size };
    tree->set_collision_box({ 120.667, 90.3333, 14.66, 20.6667 });
    m_things.emplace(tree->id(), tree);

    tree = new Object{ renderer, 100, 100, resource_blank, resource_blank_size };
    tree->set_collision_box({ 16.6667, 106.333, 39.66, 21});
    ...
    for (int i = 0; i < 8; ++i)
    {
        auto *skeleton = new Skeleton{ renderer, m_sound };
        skeleton->x() += 120 + i * 64;
        skeleton->y() += 120 + i * 45;

        skeleton->set_aggravated_by(*hero);

        m_things.emplace(skeleton->id(), skeleton);
    }
