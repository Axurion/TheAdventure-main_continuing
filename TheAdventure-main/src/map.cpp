...
{
    auto map_file = std::ifstream{ "dist/mapa.tmx" };
    if (!map_file.is_open())
    {
        std::cerr << "Failed to open dist/mapa.tmx" << '\n';
        std::terminate();
    }
    ...
    auto map_img_file = std::ifstream{ "dist/mapa.png", std::ios::binary };
    if (!map_img_file.is_open())
    {
        std::cerr << "Failed to open dist/mapa.png" << '\n';
        std::terminate();
    }
